...
class FlutterNearbyConnectionsPlugin : FlutterPlugin, MethodCallHandler, ActivityAware {
    ...
    companion object {
        // PARCHEADO: acá había un `registerWith(registrar: Registrar)` para
        // el embedding v1 de Flutter (pre-2019), que el engine actual ya ni
        // siquiera llama — `Registrar` fue eliminado del SDK. El registro
        // real pasa por `FlutterPlugin`/`ActivityAware` arriba, que ya
        // estaba implementado. Ver README, sección "Paso 4".
        ...
        try {
            when (call.method) {
                ...
                startAdvertisingPeer -> {
                    ...
                    val deviceName = if (call.argument<String>("deviceName").isNullOrEmpty())
                        localDeviceName
                    else
                        call.argument<String>("deviceName")!!
                    serviceBindManager.mService?.startAdvertising(strategy, deviceName)
                    result.success(true)
                }
            ...
            Log.e("nearby_connections", "Method ${call.method} threw an exception", e);
            result.error("exception", "Method ${call.method} threw an exception", e);
        }
    }
